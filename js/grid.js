...
class DifficultyRamp {
    ...
    dump(lvl) {
        if (lvl % 3 == 0) {
            const w = f.randomChoice(this.grid.weather.systems.filter(w => w.type == 'dump'));
            if (w.time == Infinity) 
                w.time = 1000;
            else
    ...
    point(lvl) {
        if (lvl % 3 == 2) {
            const w = f.randomChoice(this.grid.weather.systems.filter(w => w.type == 'point'));
            if (w.time == Infinity) 
                w.time = 600;
            else
                w.time = Math.ceil(w.time*0.95);
    ...
    single(lvl) {
        if (lvl % 3 == 1) {
            const w = f.randomChoice(this.grid.weather.systems.filter(w => w.type == 'single'));
            w.time = Math.ceil(w.time*0.95);
            return true; // found
        }
        return false; // not found
    ...
    special(lvl) {
        switch(lvl) {
            case 10: unlocked = 7; break;
            case 20: unlocked = 8; break;
...
export class Grid {
    constructor(params) {
        this.center = f.point(params.dim.w/2, params.dim.h/2);
        this.params = {...params};
        this.EMPTYTOP = -300;
        this.TOPPADDING = 200;
        this.MAXBLASTS = 9;
        ...
        this.blasts = new C.ImageCounterControl({
            pos: {x: 20, y: 30},
            dim: {w: 30, h: 30},
            fontSize: 20,
            text: 'Blasts:',
        ...
        this.survive = new C.TextControl({
            pos: {x: this.params.dim.w-120, y: 30},
            fontSize: 20,
            text: 'Time:',
            ctx: this.params.ctx
        });
        this.pause = new C.ButtonControl({
        ...
        this.boxes = {
            blasts: new C.BoxControl({
                pos: f.point(0,15), 
                dim: f.dimension(this.params.dim.w, 40), 
                bgColor: '#f7f', 
                bgAlpha: 0.3
            }),
            pause: new C.PauseOverlay({
                pos: f.point(0,58),
                dim: f.dimension(this.params.dim.w, 65),
                bgColor: '#77f',
                bgAlpha: 0.3,
                text: 'Paused',
                fontSize: 28,
            ...
            lost: new C.LostOverlay({
                pos: f.point(0,150),
                dim: f.dimension(this.params.dim.w, 175),
                bgColor: '#f55',
                bgAlpha: 0.7,
                color: '#fff',
                text: 'You lost!',
                ...
                imgDim: f.dimension(50,50),
                button: new C.ButtonControl({
                    text: 'Play again',
                    pos: f.point(0,0),
                    dim: f.dimension(120,30),
                    fontSize: 16,
                    bgColor: '#77f',
                    color: '#fff',
                    ctx: this.params.ctx,
            ...
            intro: new C.IntroOverlay({
                pos: f.point(0,150),
                dim: f.dimension(this.params.dim.w, 200),
                img: this.params.assets['spacematch'],
                imgDim: f.dimension(350, 60),
                text: 'Click to start',
                fontSize: 28,
    ...
    assignTypes() {
        // Offscreen get no types
        const centers = this.centers.filter(c => !c.poly.empty);
        centers.sort((a,b) => {
            const [ai,aj] = f.strpair(a.pairstr);
            const [bi,bj] = f.strpair(b.pairstr);
            if (ai-bi != 0) return ai-bi;
            if (aj-bj != 0) return aj-bj;
            return -1;
        });
        centers.forEach(c => {
            const taken = c.neighbors.map(n => n.poly.params.type);
            const start = f.randint(0, unlocked);
            for (let i=0; i<unlocked; i++) {
                const idx = (start+i)%unlocked;
                if (!taken.includes(types[idx])) {
                    c.poly.params.type = types[idx];
                    c.poly.params.image = this.params.assets[types[idx]];
                    c.poly.params.color = colors[idx];
                    return;
                }
            }
            console.log('bad type assign');
            c.poly.params.color = '#000';
        });
    }
    ...
    findNeighbors(c) {
        const [i,j] = f.strpair(c.pairstr);
        const ns = [];
        for (let ii=i-1; ii<=i+1; ii++) {
            for (let jj=j-1; jj<=j+1; jj++) {
                try { // Some indices are outside of grid
                    this.map[f.pairstr([ii,jj])].forEach(cc => {
                        const d = f.len(f.sub(c,cc));
    ...
    initHex() {
        ...
        for (let i=this.params.irange[0]; i<=this.params.irange[1]; i++) {
            for (let j=this.params.jrange[0]; j<=this.params.jrange[1]; j++) {
                ...
                const c = f.rotate(f.point(x, y), this.params.angle);
                const str = f.pairstr([i,j]);
                c.pairstr = str;
                this.map[str] = [c];
            }
        }
    ...
    get matched() {
        function growGroupTri(group, visited) {
            group.at(-1).neighbors.forEach(n => {
                if (!n.poly.empty && group.at(-1).poly.params.color == n.poly.params.color && !visited.includes(n)) {
                    group.push(n);
                    visited.push(n);
        ...
        function growGroup(group, c, i, j, di, dj) {
            ...
            const npairstr = f.pairstr([ii,jj]);
            c.neighbors.forEach(n => {
                if (!n.poly.empty && c.poly.params.color == n.poly.params.color && n.pairstr == npairstr) {
                    group.push(n);
    ...
    // Give poly a random astron
    setRandomAstron(poly) {
        const idx = f.randint(0,unlocked);
        poly.params.color = colors[idx];
        poly.params.type = types[idx];
        poly.params.image = this.params.assets[poly.params.type];
    }
    ...
    shower() {
        ...
        this.weather.systems
            .filter(w => this.time % w.time == 0).forEach(w => {
                ...
                let idcs;
                if (w.type == 'point') {
                    const pc = f.randomChoice(aboveCopy);
                    aboveCopy.sort((a,b) => f.len(f.sub(a,pc)) < f.len(f.sub(b,pc)));
